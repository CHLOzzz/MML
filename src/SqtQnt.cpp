...
struct SqtQntWidget : ModuleWidget {
	SqtQntWidget(SqtQnt* module) {
        ...
        setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/SQT_QNT_resized.svg")));
        ...
        addInput(createInput<PJ301MPort>(Vec(RACK_GRID_WIDTH-4, 270), module, SqtQnt::INPUT));

        // Add output voltage to module
        addOutput(createOutput<DarkPJ301MPort>(Vec(RACK_GRID_WIDTH-4, 320), module, SqtQnt::OUTPUT));
	}
};
